...
public class App {
    ...
    private static void printAccountMenu(Session session) {
        ...
        System.out.println("  [8] Logout all sessions");
    ...
    private static void handleOpenAccount(Scanner scanner, Session session) {
        ...
        String rawTypeInput = readTrimmedLine(scanner);
        if (rawTypeInput == null) return;
        String rawType = rawTypeInput.toUpperCase();

        BankAccount.AccountType accountType;
        try {
            accountType = BankAccount.AccountType.valueOf(rawType);
